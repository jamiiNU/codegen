 ...
 * @file     usbd_class.c
 * @version  V1.00
 * @brief    NUC1262 series USBD sample file
 *
 * @note
...
void MSC_Init(void)
{
    ...
    g_i32TotalSectors = DATA_STORAGE_SIZE / UDC_SECTOR_SIZE;
...
{
    USBD_MemCopy((uint8_t *)buffer, (uint8_t *)(addr + MASS_STORAGE_OFFSET), size);
}


void DataFlashWrite(uint32_t addr, uint32_t size, uint32_t buffer)
{
    USBD_MemCopy((uint8_t *)(addr + MASS_STORAGE_OFFSET), (uint8_t *)buffer, size);
}
