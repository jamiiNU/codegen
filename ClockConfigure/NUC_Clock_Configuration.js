...
(function () {
	...
	function createD3ClockTree(inputModule) {
		...
		populateD3Tree = function (targetNodes) {
			...
			for (ii = 0, imax = targetNodes.length; ii < imax; ii += 1) {
				...
				else if (localJsonData.name === 'LIRC') {
		...
		check_d3node = function (currentNodeName, bCreateCanvas) {
			...
			if (typeof NUTOOL_CLOCK.g_Module[currentNodeName] !== 'undefined') {
				...
				else if (currentNodeName === 'CLKO_1Hz') { // for M451, M480, M0564, NUC126 and NUC200AE
		...
		uncheck_d3node = function (currentNodeName, chainEffectException) {
			...
			if (typeof NUTOOL_CLOCK.g_Module[currentNodeName] !== 'undefined') {
				...
				else if (currentNodeName === 'RTC') { // for M451, M480, M0564, NUC126 and NUC200AE
		...
		rootChildren.push(sPLL, 'PLL2', 'PLL480M', 'APLL', 'HSUSB_OTG_PHY', sHCLK, sPCLK, 'PCLK0', 'PCLK1', 'CPUCLK');
		...
		dragOngoing = false;
		// size of the diagram
		viewerWidth = $tabs.width();//g_NUC_TreeView_Height - 8;
		viewerHeight = g_NUC_TreeView_Height;//$tabs.height();//g_Dialog_Width - g_NUC_TreeView_Width - 8;
		...
		tip = d3.tip().attr('class', 'd3-tip').html(function (d) {
			...
			if (!dragOngoing) {
				...
				else if (moduleName === sPLL || moduleName === 'APLL') {
					enableFieldArray = ['PD', 'OE', 'PLLEN'];
		...
		function click(d) {
			...
			if (d.enabled) {
				...
				if (d.name === 'RTC' &&
					...
					// for M451, M480, M0564, NUC126 and NUC200AE
		...
		function dblclick(d) {
			...
			if ($.inArray(d.name, rootChildren) === -1) {
				...
				if (d.name.indexOf('LIRC') === 0 || d.name.indexOf(sHIRC) === 0 || d.name.indexOf('HIRC48') === 0 || d.name.indexOf('MIRC') === 0 || d.name.indexOf('LXT') === 0 || d.name.indexOf(sHXT) === 0 || d.name.indexOf('RTC32k') === 0) {
					$("#tabs").tabs({ active: 0 });
				}
	...
	function createModuleCanvas(hostDivString, moduleName, defaultValue, selectField, dividerField) {
		...
		drawModuleDiagram = function () {
			...
			$("#" + moduleName + "_div_protection")[0].setAttribute('style', 'background-color:#ECEADF; position:absolute; left:0px; top:' + (50) + 'px; width:' + (30 + canvasWidth * 4 / 3 + 2) + 'px;height:' + (20 + canvasHeight + 60) + 'px;');
			$("#" + moduleName + "_div_showRealFreq")[0].setAttribute('style', 'background-color: #ECEADF; position:absolute; left:30px; top:' + (70 + canvasHeight + 10 + 10) + 'px;');
		...
		if (selectRegister.indexOf('DIV+') !== -1) {
			...
			selectRegister.indexOf('LIRC') === 0 || selectRegister.indexOf(sHIRC) === 0 ||
			...
			selectRegister.indexOf('PLL480M') === 0 || selectRegister.indexOf('APLL') === 0 || selectRegister.indexOf('HSUSB_OTG_PHY') === 0 ||
			selectRegister.indexOf(sPCLK) === 0 || selectRegister.indexOf('PCLK0') === 0 || selectRegister.indexOf('PCLK1') === 0) {
			allowedInput.push(selectRegister);
		}
	...
	function createCLKOCanvas(hostDivString, moduleName, defaultValue, selectField, dividerField) {
		...
		drawModuleDiagram = function () {
			...
			$("#" + moduleName + "_div_protection")[0].setAttribute('style', 'background-color:#ECEADF; position:absolute; left:0px; top:' + (50) + 'px; width:' + (30 + canvasWidth * 4 / 3 + 2) + 'px;height:' + (20 + canvasHeight + 60) + 'px;');
			$("#" + moduleName + "_div_showRealFreq")[0].setAttribute('style', 'background-color: #ECEADF; position:absolute; top:' + (70 + canvasHeight + 10 + 10) + 'px;');
			$("#" + moduleName + "_span_showRealFreq").css('color', '#2E2EFE');
	...
	function createModuleOnly(hostDivString, moduleName, defaultValue, selectField) {
		...
		if (selectRegister.indexOf(sHCLK) === 0 || selectRegister.indexOf('RTC32k') === 0 ||
			...
			selectRegister.indexOf(sPLL) === 0 || selectRegister.indexOf('PLL2') === 0 ||
			selectRegister.indexOf('PLL480M') === 0 || selectRegister.indexOf('APLL') === 0 || selectRegister.indexOf('HSUSB_OTG_PHY') === 0 ||
			selectRegister.indexOf('PCLK0') === 0 || selectRegister.indexOf('PCLK1') === 0) {
			allowedInput.push(selectRegister);
		}
		else if (selectRegister === 'HCLK_S'.toEquivalent()) {
			allowedInput.push(sHCLK);
	...
	function buildModuleTab() {
		...
		$("#tabs").append("<div id='tab-4' style='overflow: hidden;'></div>");
		//$("#tabs").css('width', '100%');
		//$("#tabs").css('height', '100%');
	...
	function buildPLLclockTab(command) {
		...
		buildPLLCTLtable = function (expectationFrequency, bClickFromRegister, inaccuracy, registerName) {
			...
			if (typeof (inaccuracy) !== 'undefined') {
				...
				if (formulaType === "NANO100AN_BN" || formulaType === "NANO112_103" || formulaType === "KM1M7") {
		...
		add_tab_2_handler = function () {
			...
			if (!g_bPressEnter) {
				...
				else if (!isFieldBe1(sOSC10K_EN) && !isFieldBe1(sOSC22M_EN) && !isFieldBe1('HIRC1EN') && !isFieldBe1('HIRC48EN') &&
		...
		// initialization
		if ((isFieldBe1(sXTL12M_EN) || isFieldBe1(sOSC22M_EN) || isFieldBe1('HIRC1EN') || isFieldBe1('HIRC48EN') || isFieldBe1('MIRCEN')) &&
			(g_realPLLoutputClock > 0 || g_realAPLLoutputClock > 0 || g_finalStep === 1 || typeof NUTOOL_PER !== 'undefined')) {
		...
		if (command !== 'single' && g_finalStep > 2) {
			if (isFieldBe1(sOSC10K_EN) || isFieldBe1(sOSC22M_EN) || isFieldBe1('HIRC1EN') || isFieldBe1('HIRC48EN') ||
	...
	function buildRefClockTab(command) {
		...
		// build the infrastructure
		removeAlldialogs();
		$tabs[0].setAttribute('style', 'width:' + (g_Dialog_Width - g_NUC_TreeView_Width - 8) + 'px; height:' + (g_Dialog_Height - 8) + 'px; border: 1px solid white; position:absolute; left:' + (g_NUC_TreeView_Width + 8) + 'px; top:0px;');
		...
		if (_.filter(NUTOOL_CLOCK.g_register_map[sPWRCON], function(s) { return s.indexOf(sXTL12M_EN + ':') !== -1; }).length !== 0) {
			if (NUTOOL_CLOCK.g_CLKSEL.hasOwnProperty(sHXTWAIT)) {
		...
		$("#" + sXTL12M_EN + "_input").change(function () {
			if (typeof (this.value) === 'undefined' || this.value === '' || !(/^\d+$/.test(this.value)) || parseInt(this.value, 10) < parseInt(NUTOOL_CLOCK.g_HXTRange.slicePriorToX('~').slicePriorToX('MHz'), 10) || parseInt(this.value, 10) > parseInt(NUTOOL_CLOCK.g_HXTRange.sliceAfterX('~').slicePriorToX('MHz'), 10)) {
				$("#" + sXTL12M_EN + "_input").val(NUTOOL_CLOCK.g_HXTfrequency / 1000000);
	...
	function buildChipTypeSelect() {
		...
		$('#ChipType')[0].setAttribute('style', 'background-color: #FFFFFF; width:' + (g_NUC_TreeView_Width - 16) + 'px; border-right: 16px solid #F2F2F2; border-bottom: 5px solid #F2F2F2;');
		$('#MCU')[0].setAttribute('style', 'background-color: #FFFFFF; width:' + (g_NUC_TreeView_Width - 16) + 'px; border-right: 16px solid #F2F2F2; border-bottom: 5px solid #F2F2F2;');
	...
	function buildClockRegsTree() {
		...
		$clockRegsTree[0].setAttribute('style', 'background-color: #FFFFFF; float:left; width:' + (g_NUC_TreeView_Width - 16) + 'px; height: ' + ((local_clockRegisterNames.length) * 20) + 'px; border-right: 16px solid #F2F2F2; border-bottom: 5px solid #F2F2F2;');
		...
		$clockRegsTree.jstree({
			...
			if (newText.indexOf(sPLL) !== -1) {
				...
				saveCurrentConfig();
				try { window.external.saveNu_configFromJS(); } catch (err) { }
		...
		$searchModule[0].setAttribute('style', 'background-color: #FFFFFF; border-right: 16px solid #F2F2F2; float:left; width:' + (g_NUC_TreeView_Width - 16) + 'px; height: 22px;');
	...
	function decideChipTypeAndClockRegs() {
		...
		if (NUTOOL_CLOCK.g_readConfigFilePath === "") {
			readConfigFilePath = "Nu_config.cfg "; // the default path of auto-restore config file
		}
		else {
			readConfigFilePath = 'file:///' + NUTOOL_CLOCK.g_readConfigFilePath;
		}
	...
	function findBusClock(targetModule) {
		...
		if (targetModule !== 'SYSTICK' && targetModule.indexOf('CLKO') !== 0 && targetModule.indexOf('CKO') !== 0) {
			...
			buttons: [
				{
					...
					click: function () {
						...
						try { window.external.closeFromJS(); } catch (err) { }
					}
				},
				{
					text: buttonCanel,
						...
						{ text: okButton,
							click: function () {
								if (checkClockConfigureCorrectness(showClockConfigureError)) {
									try { window.external.saveConfigFromJS("NUC_BROWSE_PROJECT_PATH"); } catch (err) { }
								}
	...
	function isFieldBe1(fieldName, specificRegister) {
		...
		fieldAlwaysBe1[sOSC22M_EN] = ['KM1M7AF', 'KM1M7BF'];
	...
	function concatenate_generated_code_begin(command) {
		...
		if (g_chipType === "M451") {
			...
			enableFieldDefines = ['none', 'none', 'none', 'CLK_EnableHXO(HXO_WCYCLE_' + _.filter(NUTOOL_CLOCK.g_CLKSEL[sHXTWAIT], function(s){ return s.indexOf(":" + readValueFromClockRegs(sHXTWAIT).toString()) !== -1; })[0].toString().slicePriorToX(':') + ');', 'none', 'none'];
			statusFieldDefines = ['none', 'none', 'none', 'CLK_WaitHXOReady();\n\n    /* Disable ' + sPLL +  ' circuit */\n    CLK_DisablePLL();', 'none', 'none', 'none'];
		}
		...
		if (g_bHasBSPtoSupport) {
			...
			// the main part
			g_clockRegsString += '\n';
			if (typeof NUTOOL_PER === 'undefined' && g_chipType !== 'NUC505' && g_chipType.indexOf('KM1M7') !== 0) {
	...
	function loadConfig() {
		showWarningForSaving(function () {
			try { window.external.loadConfigFromJS(); } catch (err) { }
		});
	}
	...
	function generateCode(mode) {
				...
				buttons: [
					{
						...
						click: function () {
							...
							if (validateDialogNameAndPath($('#inputText_projectName').val(), $("#generateCodeDialogInput").val())) {
								try { external.generateCodeFromJS($('#inputText_projectName').val(), $("#generateCodeDialogInput").val(), "CheckModularizeCode:" + $("#checkbox_ModularizeCode").is(":checked")); } catch (err) { }
								// close the last dialog
		...
		$('#generateCodeDialogBrowseFolder').click(function() {
				...
				try { external.generateCodeFromJS($('#inputText_projectName').val(), "generateCodeDialogInput:none", "CheckModularizeCode:" + $("#checkbox_ModularizeCode").is(":checked")); } catch (err) { }
							...
							click: function () {
								...
								$("input").each(function () {
									if (this.checked === true && $(this).val() !== "on") {
										...
										try { window.external.saveUIlanguage("UIlanguage:" + g_userSelectUIlanguage); } catch (err) { }
	...
	function loadClockConfigureTool() {
		...
		if (typeof NUTOOL_PER === 'undefined') {
			if (typeof NUTOOL_CLOCK.g_toolName !== 'undefined' && NUTOOL_CLOCK.g_toolName !== '') {
				try { external.changeDialogContentFromJS('dialogCaption:' + NUTOOL_CLOCK.g_toolName); } catch (err) { }
			}
		}
	...
	NUTOOL_CLOCK = {
		...
		getg_toolName: function () {
			return NUTOOL_CLOCK.g_toolName;
		},
		////for functional test////
